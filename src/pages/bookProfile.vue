<template>
  <div>
    <el-dialog title="上传图书" :visible.sync="dialogTableVisible" width="30%">
      <el-upload
        class="upload-demo"
        ref="upload"
        action="doUpload"
      ...
      >
        <el-button slot="trigger" size="small" type="primary">上传图书封面</el-button>
...
export default {
  ...
  methods: {
    ...
    submitUpload() {
      ...
      this.$axios
        .post(
          `/uploadBook` + "?name=" + this.pdfName + "&user=" +window.sessionStorage.login,
          fileFormData,
          requestConfig
        )
        .then(res => {
